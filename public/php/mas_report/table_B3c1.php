...
while($outstanding_results = mysql_fetch_array($outstanding_query)) {
    ...
    $out_initial_pt = supervisorGradeWisePoint($outstanding_results['initial_grade']);
    $out_final_pt = supervisorGradeWisePoint($outstanding_results['final_grade']);
    ...
    if ($outstanding_results['appeal_status'] == 'Closed') {
        ...
        if ($outstanding_results['appeal_result'] == 'Successful') {
            if ($out_final_pt == $out_initial_pt) { // unchanged
                ...
                if ($outstanding_results['final_grade'] == 'Good') {

                    if ($outstanding_results['initial_grade'] == 'Satisfactory'){
                        $satisfy_to_good++;
                    } else if($outstanding_results['initial_grade'] == 'Fair') {
                        $fair_to_good++;
                    } else if($outstanding_results['initial_grade'] == 'Unsatisfactory') {
                        $unsatisfy_to_good++;
                    }

                } else if ($outstanding_results['final_grade'] == 'Satisfactory') {

                    if ($outstanding_results['initial_grade'] == 'Fair') {
                        $fair_to_satisfy++;
                    } else if ($outstanding_results['initial_grade'] == 'Unsatisfactory') {
                        $unsatisfy_to_satisfy++;
                    }

                } else if($outstanding_results['final_grade'] == 'Fair') {

                    if ($outstanding_results['initial_grade'] == 'Unsatisfactory') {
                        $unsatisfy_to_fair++;
                    }
                }
...
//echo '<br/><br/> ====== <br/>';
//echo $outstanding_success_unchanged_grade_and_perc; echo '<br/>';
//echo $outstanding_success_better_perc_with_same_grade; echo '<br/>';
//echo $outstanding_success_better_grade; echo '<br/>';
//echo $outstanding_success_lower_grade; echo '<br/>';
//echo $outstanding_rejected = $outstanding_pending; echo '<br/>';
//echo $outstanding_returned_amount; echo '<br/>';
...
$table->addRow(50);
$table->addCell($w1)->addText("Total number of outstanding appeals by supervisors at the beginning of the measurement quarter", 'BoldText', 'pS_nospaceafter');
...
$table->addRow(50);
$table->addCell($w1)->addText("    1) Number of successful appeals which involve: ", 'BoldText', 'pS_nospaceafter');
...
$table->addRow(50);
$table->addCell($w1)->addText("      ii) no reassignment of balanced scorecard grades but an increase in percentage entitlement to variable income"
    , null, 'pS_nospaceafter');
...
$table->addRow(50);
$table->addCell($w1)->addText("    2) Number of rejected appeals ", 'BoldText', 'pS_nospaceafter');
$table->addCell($w2)->addText($outstanding_rejected, null, 'pS_nospaceafter');

$table->addRow(50);
$table->addCell($w1)->addText("    3) Number of outstanding appeals pending final outcome ", 'BoldText', 'pS_nospaceafter');
...
while($during_results = mysql_fetch_array($during_query)) {
    ...
    $initial_pt = supervisorGradeWisePoint($during_results['initial_grade']);
    $final_pt = supervisorGradeWisePoint($during_results['final_grade']);
        ...
        if ($during_results['appeal_result'] == 'Successful') {
            if ($final_pt == $initial_pt) { // unchanged
                ...
                if ($during_results['final_percent'] == $during_results['initial_percent']) {
                    ...
                    $during_success_lower_perc_with_same_grade++; //ot
                }
