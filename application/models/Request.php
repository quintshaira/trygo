...
class Request extends Zend_Db_Table{
    ...
    public function add_request($request_status_id,
                                $driver_task_id,
                                $last_cust_id,
                                $package_id,
                                $vehicle_id,
    ...
    public function update_request($request_status_id,
                                $driver_task_id,
                                $last_cust_id,
                                $package_id,
                                $vehicle_id,
    ...
    public function update_request1($idd,
                                    $driver_task_id,
                                    $vehicle_id,
                                    $delivery_address,
                                    $assigned_driver_1_id,
    ...
    public function get_request_list_rows($comp_id,$request_status,$ses_user1,$quick_search,$arr_limit,$arr_order)
    {
        ...
        $select->joinleft(array('trl'=>'tranzgo_rental'),'trl.rental_id=tr.vehicle_id',array('assigned_vehicle_name','rental_id','garage_type_id'));
    ...
    public function check_exist_drivers1($rent_from,$rent_to,$company_id,$didd)
    {
        ...
        if(count($result1) > 0)
        {
            ...
            //echo '<pre>'; print_r($not_available); exit;
            $select1 =   $this->DB->select();
            $select1->from(array('tu'=>'tranzgo_user'),
                array(
                    'user_id',
                    'user_full_name'=>new Zend_Db_Expr("CONCAT(user_fname,' ',user_lname)"),
                    'account_id'
    ...
    function get_the_driver1($req_idd)
    {
        $select = $this->DB->select();
        $select->from(array('tr'=>'tranzgo_request'), array());
        $select->joinleft(array('tu'=>'tranzgo_user'),'tu.user_id=tr.assigned_driver_1_id',
            array('user_id',
                'user_full_name'=>new Zend_Db_Expr("CONCAT(user_fname,' ',user_lname)"),
                'account_id'
                ));
    ...
    function get_the_driver2($req_idd)
    {
        $select = $this->DB->select();
        $select->from(array('tr'=>'tranzgo_request'), array());
        $select->joinleft(array('tu'=>'tranzgo_user'),'tu.user_id=tr.assigned_driver_2_id',
            array('user_id',
                'user_full_name'=>new Zend_Db_Expr("CONCAT(user_fname,' ',user_lname)"),
                'account_id'
            ));
    ...
    function get_customer_id($req_idd)
    {
        $select = $this->DB->select();
        $select->from(array('tr'=>'tranzgo_request'), array());
        $select->joinleft(array('tu'=>'tranzgo_user'),'tu.user_id=tr.assigned_driver_2_id',
    ...
    function get_customer_from_name($cust_name,$cust_contact)
    {
        $select = $this->DB->select();
    ...
    function driver_status_change_log($idd)
    {
        $select = $this->DB->select();
